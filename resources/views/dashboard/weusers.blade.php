@include('layout.dashboard.head')
            ...
            <div class="main-content">
                <div class="section__content section__content--p30">
                    <div class="container-fluid">
                        <h2 class="page-titel m-b-20">WE Users List</h2>
                        <div class="row adv-search-row">
                            <div class="col-md-5">
                                <div class="form-group">
                                    <div class="input-group">
                                        <input class="form-control py-2 border-right-0 border" onkeyup="loadcommandfun();" type="text" placeholder="Search" name="q" id="q">
                            ...
                            <div class="col-md-7 text-right pt-3 pb-2">
                                ...
                                <a href="/dashboard/weusers/add" class="btn btn-primary btn-sm">
                                    <i class="fas fa-user-plus"></i> Add WE User</a>
                            </div>
                        </div>
            ...
            <div class="modal fade" id="ColumnSetting" tabindex="-1" role="dialog" aria-labelledby="ColumnSettingLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <form name="frmid" id="frmid" action="{{ route('leads-view-header-update') }}" method="post">
                        <input type="hidden" name="module" value="we_users">
                        {{ csrf_field() }}
    ...
    <script>
        ...
        function exporttopdf(){
            var q = $("#q").val();
            window.location.href = '/dashboard/weusers/exportpdf?q='+q+'&fname='+$("#fname").val()+'&lname='+$("#lname").val()+'&role='+$("#role").val()+'&status='+$("#status").val()+'&partner='+$("#partner").val()+'&partner_type='+$("#partner_type").val()+'&phone='+$("#phone").val();
        }
        function exporttoexcel(){
            var q = $("#q").val();
            window.location.href = '/dashboard/weusers/exportexcel?q='+q+'&fname='+$("#fname").val()+'&lname='+$("#lname").val()+'&role='+$("#role").val()+'&status='+$("#status").val()+'&email='+$("#email").val()+'&phone='+$("#phone").val();
        }
        ...
        function loadcommandfun(){
            ...
            callAjax = $.ajax({
                    type: "GET",
                    url: "/dashboard/weusers/ajax",
                    //dataType : 'json',
                    cache: true,
                    data: 'q='+q+'&fname='+$("#fname").val()+'&lname='+$("#lname").val()+'&role='+$("#role").val()+'&status='+$("#status").val()+'&email='+$("#email").val()+'&phone='+$("#phone").val(),
                    beforeSend : function() {
                        if(callAjax != null) {
                            callAjax.abort();
                        }
