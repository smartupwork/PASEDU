...
class PrestashopAddMissingShopPrice extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'prestashopAddMissingShopPrice:cron';
    ...
    public function handle()
    {
        ...
        if(count($programs) > 0){
            ...
            foreach ($programs as $program) {
                ...
                if($id_shop){
                    ...
                    $product_shop = DB::connection('we_shop')
                        ...
                        ->select(['p.id_product'])
                        ->leftJoin('ps_product_shop AS ps', 'ps.id_product', '!=',  'p.id_product')
                        ->where('p.reference', '=', $program->code)
                        ->where('ps.id_shop', '=', $id_shop)
                        ->get()->first();
                    ...
                    if($product_shop){
                        ...
                        //$this->info('Shop ('.$partner_name.') Product ('.$program->code.') Price ('.$program->program_list_price.') Inserting.');
                       /* $product_shop = DB::connection('we_shop')
                            ->table('ps_product AS p')
                            ->select(['p.id_product'])
                            ->where('p.reference', '=', $program->code)
            ...
            if(count($product_shop_data) > 0){
                ...
                DB::connection('we_shop')->table('ps_product_shop')->insert($product_shop_data);
                Program::cacheClear();
